...
export default function WeeklyClassSchedule(
  { classSections, size = 'large' }: 
  { classSections: CalendarSection[], size: string }) {
...
const timeToMinutes = (timeStr: string): number => {
  ...
  const getClassPosition = (beginTime: string, endTime: string): { top: number; height: number } => {
    ...
    <div className={`w-full ${config.padding} bg-white`}>
      ...
      <div className="flex border border-gray-300 rounded-lg overflow-hidden shadow-lg">
        ...
        <div className={`flex-shrink-0 ${config.timeWidth} bg-gray-50`}>
          <div className={`${config.headerHeight} border-b border-gray-300 flex items-center justify-center font-semibold text-gray-600 ${config.headerText}`}>
            {size === 'large' ? 'Time' : ''}
          </div>
        ...
        {days.map((day, dayIndex) => (
          <div key={day} className="flex-1 border-l border-gray-300">
            <div className={`${config.headerHeight} border-b border-gray-300 flex items-center justify-center font-semibold text-gray-700 bg-gray-50 ${config.headerText}`}>
              ...
              {classesByDay[dayKeys[dayIndex]].map((section, index) => {
                ...
                return (
                  <div
                    ...
                    className={`absolute left-1 right-1 border-l-4 rounded ${config.classPadding} overflow-hidden ${classColors[section.colorCode]}`}
                    style={{
                      top: `${top}px`,
                      height: `${height}px`,
