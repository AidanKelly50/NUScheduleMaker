...
export default function CoursesDisplay() {
    ...
    const getTimeText = (section: Section) => {
        ...
        if (timesList[0].monday || timesList[0].tuesday || timesList[0].wednesday 
            || timesList[0].thursday || timesList[0].friday) {
            ...
            timesList.forEach((time) => {
                ...
                result += " " + stringToTimestring(time.beginTime) + "-" + stringToTimestring(time.endTime) + ", "
            });
            ...
            {data?.map((course) => (
                <div>
                    <div className="flex text-lg font-bold">
                        <Circle 
                            className={`${classColors[course.colorCode]} flex-shrink-0 bg-white text-white mr-1 mb-auto mt-1`} 
                            size={20} 
                        />
                    ...
                    {course.sections.map((section) => (
                        <div className="pl-7 text-[15px] mb-1.5">
                            ...
                            <span className="pl-4">
                                ...
                                Prof: {section.faculty[0] ? section.faculty[0] : "TBD"} |
                                Seats Left: {section.seatsAvailable}/{section.maximumEnrollment}
                            </span>
                        </div>
                    ))}
                </div>
